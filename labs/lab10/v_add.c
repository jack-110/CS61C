...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
     #pragma omp parallel
	{
		...
		for(int i=thread_num; i<ARRAY_SIZE; i+=threads)
			z[i] = x[i] + y[i];
	}
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
          #pragma omp parallel
	{
		int threads = omp_get_num_threads();
		int thread_num = omp_get_thread_num();
		int chunk_size = ARRAY_SIZE / threads + 1;
		int index_start = chunk_size * thread_num;
		int index_end = chunk_size * (thread_num + 1);
